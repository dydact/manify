...
def gaussian_mixture(
    ...
    if task == "classification":
        ...
        slopes = (0.5 - torch.randn(num_clusters, pm.dim, device=pm.device)) * 2
        intercepts = (0.5 - torch.randn(num_clusters, device=pm.device)) * 20
        ...
        v = N.sample((num_points,)).to(pm.device)
        labels += v
